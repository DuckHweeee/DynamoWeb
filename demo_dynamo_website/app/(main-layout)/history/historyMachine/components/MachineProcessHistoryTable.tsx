...
import { DrawingCodeProcessHistory, StaffDto } from "@/lib/type";
...
const MachineProcessHistoryTable: React.FC<MachineProcessHistoryTableProps> = ({
  ...
  const columns: ColumnDef<DrawingCodeProcessHistory>[] = [
    ...
    },
    {
      ...
      header: () => <div className="font-semibold text-xs">ĐT Sản phẩm</div>,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: () => (
        <div className="font-semibold text-xs text-center">TT Nguyên công</div>
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: () => (
        <div className="font-semibold text-xs text-center">TT Sản phẩm</div>
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: () => <div className="font-semibold text-xs">Nhân viên</div>,
      cell: ({ row }) => {
                    ...
                    table.getRowModel().rows.map((row) => (
                      ...
                      >
                        {row.getVisibleCells().map((cell) => (
                          <TableCell
                            key={cell.id}
                            className="py-7     px-4 text-sm whitespace-nowrap"
                          >
                            {flexRender(
