...
export default function Home() {
  ...
  function transformData<T extends Record<string, any>>(
    ...
    return staffData?.map((s) => {
      const targetVal = Number(s[targetKey] ?? 0);
      const realVal = Number(s[realKey] ?? 0);

      return {
        name: String(s.staffFullName ?? ""),
        target: isNaN(targetVal) ? 0 : targetVal,
        real: isNaN(realVal) ? 0 : realVal,
      };
    }) ?? [];
  }
  const chartConfigs = [
    ...
    <>
      <div className="m-2 px-4 py-5 bg-white rounded-[10px] shadow" >
        ...
        <div className="grid grid-cols-2 gap-4 my-5">
          {dataOverview && chartConfigs.map((cfg, idx) => (
            <DivergingBarChart
              key={idx}
