...
export default function EditGroupKPIForm({ groupKPI, onEdit, onCancel }: EditGroupKPIFormProps) {
    ...
    const handleInputChange = (field: keyof GroupKPI, value: string | number | null) => {
    ...
    const selectedGroup = groups.find(group => group.groupId === editGroupKPI.groupId)
    ...
    return (
        <div className="space-y-6 p-6">
                            ...
                            <>
                                ...
                                {selectedGroupName && (
                                    <div className="text-sm text-muted-foreground">
                                        Nhóm đã chọn: <span className="font-medium">{selectedGroupName}</span>
                                    </div>
                                )}
                            </>
                        )}
            ...
            {/* Time Period Section */}
            <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900 border-b pb-2">Thời gian</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <FlexibleCombobox
                            ...
                            onSelect={(value) => handleInputChange("year", value ? parseInt(value) : null)}
                ...
                <div className="grid grid-cols-1 gap-6">
                    {editGroupKPI.isMonth === 0 && (
                        <div className="space-y-2">
                            ...
                            <FlexibleCombobox
                                ...
                                onSelect={(value) => handleInputChange("month", value ? parseInt(value) : null)}
                    ...
                    {editGroupKPI.isMonth === 2 && (
                        <div className="space-y-2">
                            ...
                            <DatePicker
                                value={{ day: editGroupKPI.day || null, month: editGroupKPI.month || null }}
                                year={editGroupKPI.year}
                                onSelect={(day, month) => {
            ...
            <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900 border-b pb-2">Chỉ số hiệu suất</h3>
                <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            onChange={(e) => handleInputChange("workingHourGoal", e.target.value ? parseFloat(e.target.value) : null)}
                            placeholder="Nhập mục tiêu giờ làm việc"
                        />
                        <p className="text-xs text-muted-foreground">
                            Hệ thống sẽ tự động tính toán giờ làm việc thực tế và chênh lệch dựa trên mục tiêu này
                        </p>
                    </div>
                </div>
            </div>
