...
import { useGroupKPI, useGroupKPIMutations } from "../../../../hooks/useGroupKPI"
...
function getColumns({
    ...
    return [
                    ...
                    <div className="pl-5 font-medium text-[16px] text-[#888888]">
                        <div className="text-lg font-semibold text-[#074695]">
                            {groupsLoading ? (
                                ...
                                group ? group.groupName : 'Nhóm không xác định'
                            )}
                        </div>
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="pl-5 font-medium text-[16px] text-[#888888]">
                    <div className="text-lg font-semibold text-[#074695]">{row.getValue("year")}</div>
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="pl-5 font-medium text-[16px] text-[#888888]">
                        {groupKPI.isMonth === 0 ? (
                            <div className="text-lg font-semibold">Tháng {groupKPI.month}</div>
                        ) : groupKPI.isMonth === 1 ? (
                            <div className="text-lg font-semibold">Tuần {groupKPI.week}</div>
                        ) : (
                            <div className="text-lg font-semibold">Ngày {groupKPI.day}/{groupKPI.month}</div>
                        )}
                        <div className="text-sm mt-1">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${getPeriodColor(groupKPI.isMonth)}`}>
                                {groupKPI.isMonth === 0 ? "Hàng tháng" : groupKPI.isMonth === 1 ? "Hàng tuần" : "Hàng ngày"}
                            </span>
                        </div>
                    </div>
                )
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="pl-5 font-medium text-[16px] text-[#888888]">
                    <div className="text-lg">{Number(row.getValue("workingHour")).toFixed(1)}</div>
                </div>
            ),
        },
        {
...
export default function GroupKPIPage() {
    ...
    const [periodFilter, setPeriodFilter] = useState<"all" | "month" | "week" | "day">("all")
    ...
    const filteredGroupKPIs = React.useMemo(() => {
        ...
        if (periodFilter === "month") filtered = filtered.filter(kpi => kpi.isMonth === 0)
        else if (periodFilter === "week") filtered = filtered.filter(kpi => kpi.isMonth === 1)
        else if (periodFilter === "day") filtered = filtered.filter(kpi => kpi.isMonth === 2)
        ...
        if (dayFilter !== null && dayMonthFilter !== null && periodFilter === "day") {
            filtered = filtered.filter(kpi => kpi.day === dayFilter && kpi.month === dayMonthFilter)
        }

        return filtered
    }, [groupKPIs, periodFilter, yearFilter, monthFilter, weekFilter, dayFilter, dayMonthFilter])
    ...
    const hasActiveFilters = periodFilter !== "all" || yearFilter !== null || monthFilter !== null || 
                             weekFilter !== null || dayFilter !== null || dayMonthFilter !== null ||
                             groupSearchFilter !== "" || (table.getColumn("office")?.getFilterValue() as string) !== ""
            ...
            <div className="space-y-4 py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-4 pt-2 border-t">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-medium text-gray-600 tracking-wide">Loại KPI</label>
                        <Select value={periodFilter} onValueChange={(value: "all" | "month" | "week" | "day") => setPeriodFilter(value)}>
                    ...
                    {/* Year Filter */}
                    <div className="space-y-1">
                        <label className="text-xs font-medium text-gray-600 tracking-wide">Năm</label>
                        <Select value={yearFilter?.toString() || "all"} onValueChange={(value) => setYearFilter(value === "all" ? null : parseInt(value))}>
                            <SelectTrigger className="h-9">
                    ...
                    {periodFilter === "month" && (
                        <div className="space-y-1">
                            <label className="text-xs font-medium text-gray-600 tracking-wide">Tháng</label>
                            <Select value={monthFilter?.toString() || "all"} onValueChange={(value) => setMonthFilter(value === "all" ? null : parseInt(value))}>
                                <SelectTrigger className="h-9">
                    ...
                    {periodFilter === "week" && (
                        <div className="space-y-1">
                            <label className="text-xs font-medium text-gray-600 tracking-wide">Tuần</label>
                            <WeekPicker
                                value={weekFilter}
                                year={yearFilter}
                    ...
                    {periodFilter === "day" && (
                        <div className="space-y-1">
                            <label className="text-xs font-medium text-gray-600 tracking-wide">Ngày</label>
                            <DatePicker
                    ...
                    {/* Group Name Search */}
                    <div className="space-y-1">
                        <label className="text-xs font-medium text-gray-600 tracking-wide">Tên nhóm</label>
                    ...
                    {/* Office Search */}
                    <div className="space-y-1">
                        <label className="text-xs font-medium text-gray-600 tracking-wide">Văn phòng</label>
                        <div className="relative">
                            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-3.5 h-3.5" />
                            <Input
                ...
                {hasActiveFilters && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground bg-blue-50 border border-blue-200 rounded-lg px-3 py-2">
                        <div className="flex items-center gap-1">
                            ...
                            <span>Đang hiển thị {table.getFilteredRowModel().rows.length} / {filteredGroupKPIs.length} kết quả</span>
                        </div>
                        <div className="flex items-center gap-2 ml-auto">
                            {periodFilter !== "all" && (
                                <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-medium">
                                    {periodFilter === "month" ? "Theo tháng" : periodFilter === "week" ? "Theo tuần" : "Theo ngày"}
                                </span>
                            )}
                            {yearFilter && (
