...
export default function EditDailyReportForm({ report, onUpdate, onCancel }: EditDailyReportFormProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (report && report.id) {
      const formDataToSet = {
        dateTime: report.dateTime ? new Date(report.dateTime).toISOString().split('T')[0] : "",
        office: report.office || "",
        reportType: report.reportType || "", 
        hourDiff: report.hourDiff?.toString() || "",
        groupId: report.groupId || "",
      };
      setFormData(formDataToSet);
    }
  }, [report, groups]); // Add groups dependency

  const validateForm = () => {
    const newErrors = {
      dateTime: !formData.dateTime ? "Ngày không được để trống" : "",
      office: !formData.office ? "Phòng ban không được để trống" : "",
      reportType: !formData.reportType ? "Loại khai báo không được để trống" : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedReport = await updateDailyReport(report.id, reportData);
      toast.success("Cập nhật báo cáo hàng ngày thành công!");
      onUpdate(updatedReport);
    } catch (error) {
      toast.error("Lỗi khi cập nhật báo cáo hàng ngày");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Office */}
        <div className="space-y-2">
          <Label htmlFor="office" className="text-lg font-semibold text-gray-700 flex items-center">
            Phòng ban <span className="text-red-500 ml-1">*</span>
          </Label>
          <Select
            key={`office-${report?.id}-${formData.office}`}
            value={formData.office}
            onValueChange={(value) => setFormData(prev => ({ ...prev, office: value }))}
            disabled={isLoading}
          >
            <SelectTrigger className={`h-12 text-lg border-2 transition-colors ${errors.office ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}>
              <SelectValue placeholder="Chọn phòng ban" className="text-gray-500" />
            </SelectTrigger>
            <SelectContent>
        ...
        {/* Group */}
        <div className="space-y-2">
          <Label htmlFor="groupId" className="text-lg font-semibold text-gray-700 flex items-center">
            Nhóm <span className="text-red-500 ml-1">*</span>
          </Label>
          <Select
            key={`group-${report?.id}-${formData.groupId}`}
            value={formData.groupId}
            onValueChange={(value) => setFormData(prev => ({ ...prev, groupId: value }))}
            disabled={isLoading}
          >
            <SelectTrigger className={`h-12 text-lg border-2 transition-colors ${errors.groupId ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}>
              <SelectValue placeholder="Chọn nhóm" className="text-gray-500" />
            </SelectTrigger>
            <SelectContent>
              {groups?.map((group: Group) => (
                <SelectItem key={group.groupId} value={group.groupId} className="text-lg py-3 hover:bg-blue-50">
                  {group.groupName}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        {/* Report Type */}
        <div className="space-y-2">
          <Label htmlFor="reportType" className="text-lg font-semibold text-gray-700 flex items-center">
            Loại khai báo <span className="text-red-500 ml-1">*</span>
          </Label>
          <Select
            key={`reportType-${report?.id}-${formData.reportType}`}
            value={formData.reportType}
            onValueChange={handleReportTypeChange}
            disabled={isLoading}
          >
            <SelectTrigger className={`h-12 text-lg border-2 transition-colors ${errors.reportType ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}>
              <SelectValue placeholder="Chọn loại khai báo" className="text-gray-500" />
            </SelectTrigger>
            <SelectContent>
          ...
          {errors.reportType && (
            <p className="text-red-500 text-sm font-medium">{errors.reportType}</p>
          )}
        </div>

        {/* Hour Difference */}
        <div className="space-y-2">
          <Label htmlFor="hourDiff" className="text-lg font-semibold text-gray-700 flex items-center">
            Số giờ {formData.reportType && (
              <span className="ml-2 text-sm font-normal text-gray-500">
                ({formData.reportType === 'off' || formData.reportType === 'leave' ? 'Số âm' : 'Số dương'})
              </span>
            )}
          ...
          <Input
            ...
            className={`h-12 text-lg border-2 transition-colors ${errors.hourDiff ? "border-red-500 focus:border-red-500" : "border-gray-300 focus:border-blue-500"}`}
            disabled={isLoading}
          />
          {formData.reportType && formData.hourDiff && (
            <div className="flex items-center space-x-2 text-sm">
              <span className="text-gray-600">Giá trị sẽ được lưu:</span>
              <span className={`font-bold ${parseFloat(formData.hourDiff) < 0 ? 'text-red-600' : 'text-green-600'}`}>
                {formData.hourDiff} giờ
              </span>
            </div>
          )}
          {errors.hourDiff && (
            <p className="text-red-500 text-sm font-medium">{errors.hourDiff}</p>
          )}
        </div>
