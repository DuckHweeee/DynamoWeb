...
export default function MachineStatus({
  ...
  const getRemainingPgTime = (
    startTime: number,
    pgTime: number,
    status: string // đơn vị phút (minutes)
    ...
    <div className="mt-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {summaryItems.map((item, idx) => (
          <Card
            key={idx}
            className="p-4 flex flex-col items-center text-center gap-2 shadow-md mt-2"
          >
                ...
                {(() => {
                  ...
                  return (
                    <span>
                      <strong
                        className={
                          result.isOver ? "text-red-400" : "text-green-600"
                        }
                      >
                        {result.text}
                      </strong>
                ...
                {(() => {
                  ...
                  if (status != "Trống") {
                    ...
                    return (
                      <span>
                        <span className="text-xs text-gray-500">
                          PG: {formatMinutes(machine.pg)} | Đã chạy: 0 giờ 0
                          phút
                        </span>
                      </span>
                    );
